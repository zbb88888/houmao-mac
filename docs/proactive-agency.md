...
> 代码落点：`Core/Agent/`（模型 + 协议 + 决策 + 护栏 + 存储 + watcher）、`AgentDaemon.swift`、`AgentViewModel.swift`、`AgentInboxView.swift`；接线见 `houmaoApp.swift` / `GlobalHotKeyManager.swift` / `PanelSidebar.swift` / `MainViewModel.swift` / `SettingsView.swift`
...
macOS Shell
  ...
  AgentInboxView.swift 收件箱面板视图（ADR-11 壳；双击=触发建议命令，右键菜单，行内移除）
```
...
- **主开关** `isEnabled`：默认**关闭**，用户在设置里显式开启（不默默后台跑）。
...
- **收件箱面板**（窗口标题 `agent`，rail 图标 `bell.badge`「动态」）：照 ADR-11 壳。两分区——「请求我 review」（PR）/「指派给我」（issue）。行＝图标 + 标题 + 仓库 + 时间；**双击 = 触发建议命令**（`post` `/pr`/`/issue` 走聊天分析）；右键菜单＝分析 / 在浏览器打开 / 复制链接 / 移除；行内 `trash` = 移除（仅从收件箱移除，仍留在 `seen` 不再重复提醒）。空态提示 + 「刷新」+ 最近轮询时间。
...
**本期含**：GitHub watcher（指派 issue + review-requested PR）、后台轮询循环、本地通知、收件箱面板、设置项（主开关 / 间隔 / 静默时段 / GitHub watcher 开关）、纯建议护栏、三组纯函数单测、本设计文档。
...
- **手动**（需 `gh auth login`）：设置里开启主观能动性 → 收件箱「刷新」→ 指派 issue / review PR 进收件箱并弹通知 → 双击一键 `post /pr` → ghia 分析进聊天 → 移除某项 → **重启 app** 后收件箱持久化、已提醒项不重复通知。GUI 无头环境只保证编译 + 纯逻辑单测，真实 gh 联调需本地 `gh auth`。
