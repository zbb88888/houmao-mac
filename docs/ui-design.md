...
- **快捷键**
  - 当前设计为通过全局快捷键唤醒主窗口，默认为**双击 Option 键**（间隔 0.4 秒内）。
...
两个交互面**刻意**采用不同的会话模型，这是核心设计约定：

| 维度 | 临时对话框（最小输入框 `MainView`） | 聊天框（独立窗口 `ChatView`） |
|---|---|---|
| 会话性质 | **每次都是全新对话**（一次性问答） | **多轮对话，带历史** |
| 发给 LLM 的历史 | 空（`history: []`） | 之前所有轮次（`history: priorHistory`） |
| 触发路径 | `submit()` → `executeQuery(...)` | `sendChatTurn()` → `executeChatTurn(...)` |
| 上下文累积 | 不累积；上一问不影响下一问 | 累积；后续回复基于前文 |

设计意图：

- **临时对话框**面向「快问快答」——问完即走，互不干扰，不受历史污染，最省 token。
- **聊天框**面向「深入对话」——需要上下文延续（如 `/pr`、`/issue` 分析后的追问）。

代码约定（务必保持）：

- `executeQuery` 恒传 `history: []`——它是临时对话框的一次性路径，**不得**改为携带历史。
- `executeChatTurn` 传 `priorHistory`——它是聊天框的多轮路径。
- 临时对话框「升级」到聊天框时，用累积的 `oneShotTurns` 作为新会话的种子（一次性搬运，而非临时框本身带历史）。
